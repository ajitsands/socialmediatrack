...
if ($action === 'click') {
    ...
    if (empty($_SESSION[$sessionKey])) {
        ...
        $stmtRate = $db->prepare("
            SELECT p.client_id, p.cpc_rate, cl.wallet_balance, c.offer_code as campaign_code
            FROM campaigns c
            JOIN products p ON p.id = c.product_id
            LEFT JOIN users cl ON cl.id = p.client_id AND cl.role = 'client'
            WHERE c.id = ?
            ...
            if ($cpc > 0 && (float)$prodRate['wallet_balance'] >= 0.100) {
                try {
                    ...
                    $db->prepare("INSERT INTO client_wallet_transactions (client_id, amount, type, payment_method, note) VALUES (?, ?, 'debit', 'system', ?)") 
                       ->execute([$prodRate['client_id'], $cpc, 'CPC Click on Campaign: ' . ($prodRate['campaign_code'] ?? 'Campaign #' . $campaignId)]);
                    $db->commit();
                } catch (Exception $e) {
                    $db->rollBack();
                }
...
if ($action === 'convert') {
    ...
    $db   = getDB();
    $stmt = $db->prepare("SELECT c.offer_code as campaign_code, c.offer_code, c.discount_value, c.discount_type, p.product_url, p.demo_url, p.client_id, p.cpl_rate, cl.wallet_balance FROM campaigns c JOIN products p ON p.id=c.product_id LEFT JOIN users cl ON cl.id = p.client_id AND cl.role = 'client' WHERE c.id=? AND c.status='active'");
    $stmt->execute([$campaignId]);
    $camp = $stmt->fetch();
    ...
    if ($camp['client_id'] !== null) {
        ...
        if ($cpl > 0 && (float)$camp['wallet_balance'] >= 0.100) {
            try {
                ...
                $db->prepare("INSERT INTO client_wallet_transactions (client_id, amount, type, payment_method, note) VALUES (?, ?, 'debit', 'system', ?)")
                   ->execute([$camp['client_id'], $cpl, 'CPL Lead on Campaign: ' . ($camp['campaign_code'] ?? 'Campaign #' . $campaignId)]);
                $db->commit();
            } catch (Exception $e) {
                $db->rollBack();
            }
